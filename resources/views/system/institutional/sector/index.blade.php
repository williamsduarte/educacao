                ...
                <div class="panel panel-primary filterable" style="border:0; width:90%; margin-left: 3.5em;">
                    <div class="panel-heading" style="background: transparent">
                        <div class="pull-right" style="background: transparent">
                            <button class="btn btn-default btn-xs btn-filter"><i class="ti-search"></i> Buscar
                            </button>
                        </div>
                    </div>
                    <table class="table table-hover table-rwd-name table-hidden">
                        <thead>
                        <tr class="filters">
                            <th><input type="text" class="form-control" placeholder="Prefeitura" disabled></th>
                            <th><input type="text" class="form-control" placeholder="Secretaria" disabled></th>
                            <th><input type="text" class="form-control" placeholder="Setor" disabled></th>
                        ...
                        <tbody>
                        @forelse($sector as $sector)
                            <tr>
                                <td width="17%">{{$sector->Secretary->Prefecture->name}}</td>
                                <td width="17%">{{$sector->Secretary->name}}</td>
                                <td width="17%">{{$sector->sector}}</td>
                                <td width="17%">{{$sector->branch}}</td>
                                <td width="17%"><a href="{{route('sector.edit', $sector->id)}}" style="color: #000;"><i class="ti-pencil"></i></a></td>
                                <td width="">
                                    <form method="POST" action="{{route('sector.destroy', $sector->id)}}" style="margin: 0; width:0; padding: 0;">
                                        {!! method_field ('DELETE') !!}
                                        {!! csrf_field() !!}
                                        <button style="margin: 0; padding:0; border:0; background: transparent;"><i
