            ...
            <div class="row">
                <div class="panel panel-primary filterable" style="border:0; width:90%; margin-left: 3.5em;">
                    ...
                    <table class="table table-hover table-rwd-name table-hidden">
                        <thead>
                        <tr class="filters">
                            <th><input type="text" class="form-control" placeholder="Escola" disabled></th>
                            <th><input type="text" class="form-control" placeholder="Ano Letivo" disabled></th>
                            <th><input type="text" class="form-control" placeholder="Início" disabled></th>
                        ...
                        @forelse($lesson as $lesson)
                            <tr>
                                <td width="17%">{{$lesson->School->LegalPerson->fantasy_name}}</td>
                                <td width="17%">{{$lesson->year}}</td>
                                <td width="17%">{{date('d-m-Y', strtotime($lesson->start))}}</td>
                                <td width="17%">{{$lesson->status}}</td>
                                <td width="17%"><a href="{{route('lesson.edit', $lesson->id)}}" style="color: #000;"><i class="ti-pencil"></i></a></td>
                                <td width="17%">
                                    <form method="POST" action="{{route('lesson.destroy', $lesson->id)}}" style="margin: 0; width:0; padding: 0;">
                                        {!! method_field ('DELETE') !!}
                                        {!! csrf_field() !!}
